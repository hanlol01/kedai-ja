...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate file size (max 100MB for development, 50MB for production)
    const isProduction = process.env.NODE_ENV === 'production';
    const maxSize = isProduction ? 50 * 1024 * 1024 : 100 * 1024 * 1024;
    
    if (file.size > maxSize) {
      const maxSizeMB = Math.round(maxSize / (1024 * 1024));
      return NextResponse.json({ 
        error: `Video terlalu besar (maksimal ${maxSizeMB}MB)` 
      }, { status: 413 });
    }
    ...
    console.log(`Video saved: ${fileName} (${file.size} bytes)`);
