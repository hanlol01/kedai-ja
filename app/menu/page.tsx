...
export default function Menu() {
  ...
  const [modalImageData, setModalImageData] = useState<{src: string, name: string, description: string} | null>(null);
  ...
  const getAvailableSubCategories = () => {
    ...
    if (selectedCategory === 'Makanan' || selectedCategory === 'Minuman') {
  ...
  // Handle image modal
  const openImageModal = (item: MenuItem) => {
    console.log('openImageModal called with:', item.name);
    setModalImageData({
      src: item.image || '/logo-bg.png',
      name: item.name,
  ...
  return (
    <MainLayout>
          ...
          {getAvailableSubCategories().length > 0 && (
            <div className="flex flex-wrap justify-center gap-2">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300 transform hover:scale-105 ${
                  selectedSubCategory === 'all'
                    ...
                    : 'bg-white/5 text-gray-300 border border-white/10 hover:border-secondary-400/50 hover:text-secondary-300'
                }`}
              >
                Semua
              </button>
              ...
              {selectedSubCategory !== 'all' && (
                <button
                  onClick={() => setSelectedSubCategory('all')}
                  className="ml-2 p-2 rounded-full bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-all duration-300"
                  title="Reset filter sub kategori"
                >
                  <X className="h-4 w-4" />
                </button>
              )}
        ...
        {filteredItems.length === 0 ? (
          ...
          <>
                        ...
                        >
                          {item.image ? (
                            <img 
                              src={item.image} 
                              alt={item.name} 
                              className="w-full h-full object-cover group-hover/image:scale-110 transition-transform duration-300 rounded" 
                            />
                          ) : (
                            <div className="w-full h-full bg-gradient-to-br from-primary-400 to-secondary-400 flex items-center justify-center rounded">
                              <ChefHat className="h-8 w-8 text-white" />
                            </div>
                          )}
                          <div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent opacity-0 group-hover/image:opacity-100 transition-opacity duration-300 rounded flex items-center justify-center">
                        ...
                        <div>
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <div className="flex flex-col gap-1 ml-2 flex-shrink-0">
                              ...
                              {item.subCategory && (
                                <span className="px-2 py-1 rounded-full text-xs font-medium bg-gray-800/70 text-gray-300">
                                  {item.subCategory}
                                </span>
                              )}
                            </div>
            ...
            <div className="hidden lg:block">
              {/* Test Button - Only visible on desktop */}
              <div className="mb-6 text-center">
                <button
                  onClick={() => {
                    console.log('Desktop test button clicked');
                    openImageModal({
                      _id: 'test',
                      name: 'Test Desktop Modal',
                      description: 'Testing modal functionality on desktop',
                      price: 0,
                      category: 'Makanan',
                      subCategory: 'Test',
                      image: '/logo-bg.png',
              ...
              {filteredItems.map((item, index) => (
                <div key={item._id} 
                  ...
                  <div 
                    ...
                    onClick={(e) => {
                      console.log('Desktop click triggered for:', item.name);
                      e.stopPropagation();
                      openImageModal(item);
                    }}
                  >
                    {item.image ? (
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-primary-400 via-secondary-400 to-primary-500 flex items-center justify-center">
                        <ChefHat className="h-20 w-20 text-white opacity-80 hover:scale-110 transition-transform duration-500" />
                      </div>
                    )}
                    ...
                    <div className="absolute inset-0 bg-black/20 opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                      <div className="bg-white/20 backdrop-blur-sm rounded-full px-4 py-2">
                        <span className="text-white font-medium">Klik untuk Preview</span>
                      </div>
                    </div>
                    ...
                    <div className="absolute top-4 right-4 flex flex-col gap-2 z-20 pointer-events-none">
                      ...
                      {item.subCategory && (
                        <div className="px-3 py-1 rounded-full text-xs font-medium bg-gray-900/80 text-gray-200 shadow-lg">
                          {item.subCategory}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="p-6">
                    <div className="mb-4">
                      <div className="flex items-start justify-between mb-2">
                        <h3 className="heading-secondary text-xl font-bold text-white group-hover:text-primary-300 transition-colors duration-300 flex-1">
                          {item.name}
                        </h3>
                        {item.subCategory && (
                          <span className="ml-3 px-3 py-1 rounded-full text-xs font-medium bg-gray-800/50 text-gray-300 border border-gray-700/50">
                            {item.subCategory}
                          </span>
                        )}
                      </div>
      ...
      {showImageModal && modalImageData && (
        ...
        >
          <div 
            className="relative bg-white/10 backdrop-blur-md rounded-3xl overflow-hidden max-w-4xl max-h-[90vh] w-full shadow-2xl border border-white/20"
          ...
          >
            ...
            <div className="relative">
              <img 
                ...
                alt={modalImageData.name}
                className="w-full h-auto max-h-[70vh] object-contain"
                onError={(e) => {
                  console.error('Image failed to load:', modalImageData.src);
                  e.currentTarget.src = '/logo-bg.png';
                }}
              />
            ...
            {/* Optional: Add zoom controls or additional info here */}
